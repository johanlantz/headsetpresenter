...
DWORD CCrc32Static::FileCrc32Win32(LPCTSTR szFilename, DWORD &dwCrc32)
{
	...
	dwCrc32 = ~dwCrc32;
	dwCrc32 -= 1; //Johan salting
	return dwErrorCode;
}
//***********************************************
DWORD CCrc32Static::FileCrc32Win32Mod(LPCTSTR szFilename, DWORD &dwCrc32)
{
	_ASSERTE(szFilename);
	_ASSERTE(lstrlen(szFilename));
	...
	int LI_SIZE = 3;
	FILE *fsrc;
	if((fsrc = fopen(szFilename, "rb")) == NULL)  //source file check
	{
		MessageBox(NULL,"Missing file",szFilename,MB_OK);
	}
	...
	try
	{
		...
		else
		{
			...
			DWORD dwBytesRead, dwLoop;
			BOOL bSuccess = ReadFile(hFile, buffer, sizeof(buffer), &dwBytesRead, NULL);	
			if(dwBytesRead != MAX_BUFFER_SIZE)
			{
				dwBytesRead = dwBytesRead - totalSizeOfEncryptedData - LI_SIZE;  //Added to remove encrypted tail from CRC check
			}
			while(bSuccess && dwBytesRead)
			{
				for(dwLoop = 0; dwLoop < dwBytesRead; dwLoop++)
				...
				bSuccess = ReadFile(hFile, buffer, sizeof(buffer), &dwBytesRead, NULL);
				//char t[100];
				//MessageBox(NULL,itoa(dwBytesRead,t,10),"",MB_OK);
				if(dwBytesRead == (totalSizeOfEncryptedData + LI_SIZE))
				{
					//MessageBox(NULL,"Got Rest","",MB_OK);
					dwBytesRead = dwBytesRead - totalSizeOfEncryptedData - LI_SIZE;  //Added to remove encrypted tail from CRC check, this will terminatate the while loop
				}
				else if(dwBytesRead != MAX_BUFFER_SIZE && dwBytesRead != 0)  //Perhaps 4096 must be 512 ?
				{
					MessageBox(NULL,"Potential Problem with fileSize not divided by MAX_BUFFER_SIZE ","",MB_OK);
					dwBytesRead = dwBytesRead - totalSizeOfEncryptedData - LI_SIZE;
					//4096 seems to be a magic number, atleast the dll filesize is dividable with 4096
					//Perhaps this needs to be changed if it is possible to have 512 as smallest file segment
				}
